...
test.describe('Comparer Performance Tests', () => {
  const viewName = 'Comparer';
  const url = 'http://localhost:5173/#/comparer';
  ...
  test('should compare against baseline', async ({ page }) => {
    const baseline = await loadBaseline('./tests/performance/baselines/comparer.json');
    const result = await runPerformanceTest(page, viewName, url);
