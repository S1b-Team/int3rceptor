...
test.describe('Decoder Performance Tests', () => {
  const viewName = 'Decoder';
  const url = 'http://localhost:5173/#/decoder';
  ...
  test('should compare against baseline', async ({ page }) => {
    const baseline = await loadBaseline('./tests/performance/baselines/decoder.json');
    const result = await runPerformanceTest(page, viewName, url);
