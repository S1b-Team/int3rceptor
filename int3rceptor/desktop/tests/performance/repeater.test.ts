...
test.describe('Repeater Performance Tests', () => {
  const viewName = 'Repeater';
  const url = 'http://localhost:5173/#/repeater';
  ...
  test('should compare against baseline', async ({ page }) => {
    const baseline = await loadBaseline('./tests/performance/baselines/repeater.json');
    const result = await runPerformanceTest(page, viewName, url);
