...
test.describe('Plugins Performance Tests', () => {
  const viewName = 'Plugins';
  const url = 'http://localhost:5173/#/plugins';
  ...
  test('should compare against baseline', async ({ page }) => {
    const baseline = await loadBaseline('./tests/performance/baselines/plugins.json');
    const result = await runPerformanceTest(page, viewName, url);
