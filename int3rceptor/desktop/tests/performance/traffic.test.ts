...
test.describe('Traffic Performance Tests', () => {
  const viewName = 'Traffic';
  const url = 'http://localhost:5173/#/traffic';
  ...
  test('should compare against baseline', async ({ page }) => {
    const baseline = await loadBaseline('./tests/performance/baselines/traffic.json');
    const result = await runPerformanceTest(page, viewName, url);
