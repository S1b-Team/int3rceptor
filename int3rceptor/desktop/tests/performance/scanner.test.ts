...
test.describe('Scanner Performance Tests', () => {
  const viewName = 'Scanner';
  const url = 'http://localhost:5173/#/scanner';
  ...
  test('should compare against baseline', async ({ page }) => {
    const baseline = await loadBaseline('./tests/performance/baselines/scanner.json');
    const result = await runPerformanceTest(page, viewName, url);
